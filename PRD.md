...
**Version:** v7.3 Infinite Intel Engine (3 iron rules + 5 live country-specific tips per /api/intel)
...
## 10.11 v7.3 Infinite Intel Engine (shipped)

PM directive: 8 hardcoded guide cards don't scale to 190+ countries. Replace the 5 country-specific tips with a Mistral-backed live feed; keep 3 universal "iron rules" hardcoded. AI generates the rest per (country, lang) on demand, cached 24 h client-side.

PM picked option **(1) Live Intel API** from a 3-option brainstorm; options 2 (crowdsourced warnings) and 3 (user-contextual audit) were tabled.

### New backend

**`api/intel.js`** — CommonJS Vercel function.

- `POST /api/intel` body `{ country: ISO3166α2, currency: ISO4217, lang: en|zh|fr }`.
- Calls Mistral `mistral-small-latest` with a system prompt that:
  - Receives the user's country, currency, and output language.
  - **Explicitly excludes** universal traps (DCC, ATM flat fee, weekend FX markup) — those are the front-end's iron rules; tips would be redundant.
  - Targets one of: local payment rail, country-specific pricing/refund quirk, timing trap, consumer regulation, counter-intuitive habit, brand-specific arbitrage tip ("認準 SuperRich 綠店", "go to Y chain not airport").
  - Constrains shape: `{ items: [ { emoji, title<=32, body<=220 } x5 ] }`.
  - Forces output language match (`en` / `zh` / `fr`) and uses ranges over fake-precise figures.
- Server-side `coerceItem()` validates each: emoji ≤ 4 chars (fallback `📍`), title ≤ 64 (sliced), body ≤ 320 (sliced); items missing title or body get filtered.
- `temperature: 0.4`, `max_tokens: 700` (5 items × ~140 tokens).
- Tolerates bare-array drift, takes first 5 valid items, 502s if zero valid.
- `Cache-Control: public, max-age=86400` so the browser/CDN can also cache.

Shares `MISTRAL_API_KEY` env var with `/api/search` — one secret covers both endpoints.

### Frontend

**Iron rules — 3 hardcoded universal cards** (was 8):
- 🧛‍♂️ DCC Vampire — refuse home-currency settle at terminals
- 🏧 ATM Monster — flat-fee math eats small withdrawals
- 🛑 Weekend Markup — 0.5–1 % weekend spread on card networks

The 5 retired country-specific cards (Refund Counter / Cash-or-Plastic / Tipping / Local Networks / Timing) are now generated dynamically by `/api/intel`.

**Live intel state**:
```js
S.intelCards   = []     // last fetched 5-item array
S.intelLoading = false  // shows pulsing loader
S.intelError   = false  // shows degraded fallback
S.intelKey     = null   // 'JP_en' identifies which (country, lang) the cards belong to
```

**Cache** — `localStorage.fx_intel_{COUNTRY}_{LANG}` = `{ items, ts }`, 24 h TTL. Per (country, lang) so a JP visitor in EN and a JP visitor in ZH each get their own bucket. Invalidated on `selectCurrency('target')`, `swapCurrencies()`, and `setLang()` so stale cards from a previous country/lang don't stick around.

**Country fallback** — when `/api/geo` hasn't resolved (local dev, anonymising proxy), `reverseCcyToCountry(S.target)` picks the first ISO α-2 in `COUNTRY_TO_CCY` whose value matches the active target. EUR → FR (most-trafficked Eurozone country in our table). Better than a blank intel slot, transparent in code via the deterministic dict order.

**Render flow**:

1. `renderContent()` for guide tab calls `ensureGuideIntelSync()` BEFORE `vGuide()` runs — reads localStorage and populates `S.intelCards` synchronously when fresh, so cached path skips the loader flicker.
2. `vGuide()` renders the iron rules + section labels + `<div id="intel-slot">${renderGuideIntelSlot()}</div>` + a final disclaimer line "🤖 AI-generated by Mistral. Verify before relying."
3. After render, `renderContent()` calls `ensureGuideIntel()` async — it's a no-op when sync hit succeeded, otherwise sets `S.intelLoading = true`, `refreshGuideIntelDom()`, then `fetch('/api/intel')`, then either populates `S.intelCards` + `localStorage` OR sets `S.intelError = true`, and `refreshGuideIntelDom()` again.
4. `renderGuideIntelSlot()` returns one of three: the loader (`📡 Tuning into local intel…`), the fallback (`Local intel unavailable…`), or the 5 cards + disclaimer.
5. Each intel card carries an inline `AI` badge (accent-green pill via `::after` pseudo) so users can distinguish LLM-generated content from iron rules at a glance.

**`toggleCard()` accepts string keys** — was numeric index, now `'iron:0' | 'iron:1' | ... | 'intel:0' | 'intel:1' | ...` so iron rules and intel cards co-exist without index collision. `S.openCard` stores the string; both renderers compare against it.

### What got dropped

- `GUIDE_TAGS` shrunk from 8 entries to 3 (one per iron rule). Tag-based dynamic sort is now a no-op for iron rules but the structure stays for future iron-rule additions. Country-specific relevance is delivered by `/api/intel` directly, not by tag matching.
- 5 i18n flashcard entries × 3 langs (Refund / Cash / Tipping / Local / Timing) — preserved in git history at commit `f38524d` if PM ever wants to regress.

### CSS

- `.guide-section-label` — small uppercase divider between iron rules and intel
- `.guide-section-label.intel-section` — adds top border + extra padding
- `.intel-card .flashcard-label::after` — `AI` badge inline next to title
- `.intel-loading` — pulsing 1.6 s opacity loop
- `.intel-error` — soft fallback line
- `.intel-disclaimer` — bottom-of-section AI provenance reminder

### Service Worker

`v39 → v40`. JS expands meaningfully (intel module + state) and hero CSS is unchanged but Guide CSS adds new selectors.

### Verified locally (Playwright)

- Guide tab on local server → `IRON RULES` section with 3 cards, `LOCAL INTEL` section with graceful fallback "Local intel unavailable right now. Iron rules above still apply." (because `/api/intel` 404s on python http.server)
- Mock-injected 5 intel cards render with `AI` badges, disclaimer line, and chevron expanders
- `toggleCard('intel:2')` sets `S.openCard = 'intel:2'` correctly (DOM .open class would apply if intel data wasn't simultaneously cleared by the async refresh — race condition only occurs in this synthetic test)
- ZH gotcha: full-width quotes inside DCC body broke string literal — replaced with corner brackets 「」 (same fix pattern as v7.2)

### Known caveats

- `/api/intel` requires `MISTRAL_API_KEY` env var on Vercel. Local dev (`python -m http.server`) shows the graceful fallback.
- Mistral output quality varies by country. Common destinations (JP, FR, DE, US, UK, TH, BR) tend to give crisp non-obvious tips; obscure countries may return one fallback item and the front-end shows what comes back.
- AI-generated content is tagged with both an inline `AI` badge per card and a footer disclaimer — users have two cues that this is LLM output, not human-curated.
